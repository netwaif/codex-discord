...
TUI 채널 아래 **디스코드 스레드**에 메시지가 오면 같은 tmux 세션에 창 `t<스레드ID 끝 6자리>`를 만들어
별도 codex/agy 세션을 띄우고(`scripts/tui-up.sh <env> --window t…`), 그 세션의 답만 그 스레드에 게시한다.
메인 채널 세션은 스레드 내용을 모른다. 매핑은 `DATA_DIR/threads.json`에 남아 데몬을 재시작해도 살아 있는 창에 다시 붙는다
(창이 죽었으면 다음 메시지에 새 세션). `TUI_THREADS=off`면 스레드를 무시한다(공유 채널 배치).
`tui-restart.sh`는 세션을 통째로 재생성하므로 스레드 창도 사라진다.
